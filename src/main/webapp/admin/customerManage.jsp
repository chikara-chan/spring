 	...
 	<div class="row">
 		<div class="col-sm-2">
			<div class="left-part">
				...
				<div class="sub-title"><a class="active" href="${pageContext.request.contextPath}/customer/listByAdmin">用户管理</a></div>
				...
				<table class="table">
					...
					<tbody>
						<c:forEach var="customer" items="${customers}">
							<tr>
								<td>
									<span class="strong"><fmt:formatDate pattern="yyyy/MM/dd" value="${customer.createTime}" /></span>
									<span class="text-muted small"><fmt:formatDate pattern="HH:mm" value="${customer.createTime}"/></span>
								</td>
								<td>
									<span class="strong"><fmt:formatDate pattern="yyyy/MM/dd" value="${customer.lastTime}" /></span>
									<span class="text-muted small"><fmt:formatDate pattern="HH:mm" value="${customer.lastTime}"/></span>
								</td>
								<td>${customer.name}</td>
								<td>
									<c:if test="${!empty customer.phone}">${customer.phone}</c:if>
									<c:if test="${empty customer.phone}">尚未绑定</c:if></td>
								<td>
									<c:if test="${!empty customer.email}">${customer.email}</c:if>
									<c:if test="${empty customer.email}">尚未绑定</c:if></td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
